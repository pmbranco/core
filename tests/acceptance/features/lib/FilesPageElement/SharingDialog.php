...
class SharingDialog extends OwncloudPage {
	...
	private $selectedDateXpath = "//table[@class='ui-datepicker-calendar']//*[contains(text(),'%s')]";
	...
	public function setExpirationDateFor(Session $session, $user, $type = 'user', $value = '') {
		...
		// you need to click on date of the calendar to make the calendar widget go away
		$day = \explode('-', $value)[0];
		$selectedDate = $this->find("xpath", \sprintf($this->selectedDateXpath, $day));
		$this->assertElementNotNull(
			$selectedDate,
			"Could not find the selected date for " . $type . " '$user'"
		);
		// in some cases like when enforcing expiration date and changing of expiration date is not allowed
		// calendar widget disappears after changing the date and hitting enter, so no need to click on the widget
