...
trait Sharing {
	...
	/**
	 * @Then /^the fields of the response for (user|group) ((?:[^']*)|(?:[^"]*)) for (file|folder) ((?:[^']*)|(?:[^"]*)) should include$/
	 *
	...
	public function theFieldsOfTheResponseForUserForResourceShouldInclude(
		...
		foreach ($data as $share) {
			if ($share['file_target'] === $fileName && $share['item_type'] === $fileOrFolder) {
				if (($share['share_type'] === 0 && $userOrGroup === "user")
					|| ($share['share_type'] === 1 && $userOrGroup === "group")
				) {
					$share_id = $share['id'];
				}
			}
		...
		if ($share_id === null) {
			throw new Exception("Could not find share id for " . $user);
		}
